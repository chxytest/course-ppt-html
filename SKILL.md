...
- `references/sample-prompts.md`：阶段2/3/4/5 **真实用户样例提示词参考**（总调度 / 三视图 / 文档方案 / guizang / ian / 视频），**按需展示给用户调优，不写死、不强制弹**。
...
**样例提示词**：阶段2/3 样例（文档方案模板、guizang 总调度）见 `references/sample-prompts.md`。进入阶段3 时**可按需**展示给用户调优风格与主题色——**不强制弹**，仅在用户需要或你判断有助于对齐时展示，确认后再生成。
...
**样例提示词**：阶段4 样例（IP 三视图、ian 适配、IP 配图总调度）见 `references/sample-prompts.md`。进入阶段4 时**可按需**展示给用户调优角色/动作/标注词——**不强制弹**，确认后再生成。
